(function (scope) {
  ...
  function meta(state,{project,album,revision}={}){
    ...
    // detailed=true only on a project's own page, where its videos are actually
    // embedded — keeps the homepage's hasPart list from repeating ~49 VideoObject
    // entries (one per source film) on every load.
    const work=(p,detailed)=>({'@type':'CreativeWork','@id':ORIGIN+projectUrl(p),url:ORIGIN+projectUrl(p),name:p.brand+' — '+p.title,description:'Emir Selahattin Şahin: '+p.role,inLanguage:'tr',image:abs(image(p.cover)),contributor:{'@type':'Role',roleName:p.role,contributor:{'@id':ORIGIN+'/#emir'}},...(detailed?{associatedMedia:(p.files||[]).map((f,i)=>({'@type':'VideoObject','@id':ORIGIN+projectUrl(p)+'#video'+(i+1),name:p.brand+' — '+p.title+' · Film '+(i+1),description:'Emir Selahattin Şahin: '+p.role+'. '+p.brand+' — '+p.title+' projesinden film.',thumbnailUrl:abs(image(p.cover)),contentUrl:abs(asset(f,'videos')),embedUrl:ORIGIN+projectUrl(p)+'#video'+(i+1)}))}:{})});
    ...
    const graph=[person,{'@type':'WebSite','@id':ORIGIN+'/#website',url:ORIGIN+'/',name:'Emir Selahattin Şahin',inLanguage:'tr-TR',publisher:{'@id':ORIGIN+'/#emir'}},project?work(project,true):album?gallery(album):{'@type':'ProfilePage',url:canonical,name:title,description,inLanguage:'tr-TR',mainEntity:{'@id':ORIGIN+'/#emir'},...(revision?{dateModified:revision}:{}),hasPart:state.projects.map(p=>work(p,false))}];
